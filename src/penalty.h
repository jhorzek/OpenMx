...
class SmoothLassoPenalty : public Penalty {
  typedef Penalty super;
public:
  SmoothLassoPenalty(S4 _obj, omxMatrix *_mat) : Penalty(_obj, _mat) {}
...
class SmoothRidgePenalty : public Penalty {
  typedef Penalty super;
public:
  SmoothRidgePenalty(S4 _obj, omxMatrix *_mat) : Penalty(_obj, _mat) {}
  virtual void compute(int ffcompute, FitContext *fc) override;
  virtual std::unique_ptr<Penalty> clone(omxMatrix *mat) const override;
};

class SmoothElasticNetPenalty : public Penalty {
  typedef Penalty super;
public:
  SmoothElasticNetPenalty(S4 _obj, omxMatrix *_mat) : Penalty(_obj, _mat) {}
  virtual void compute(int ffcompute, FitContext *fc) override;
  virtual std::unique_ptr<Penalty> clone(omxMatrix *mat) const override;
};
